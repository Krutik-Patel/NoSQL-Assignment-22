...
public class PairsClassLA {
	...
	public static class PairMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		private Map<String, Integer> localAggregate = new HashMap<String, Integer>();
			...
			for (Integer idx: frequentIndices) {
				...
				while(j <= d) {
					if (frequentIndices.contains(idx+j)) {
						...
						if (localAggregate.get(strPair) != null) {
							localAggregate.put(strPair, localAggregate.get(strPair)+1);
						}
						else {
							localAggregate.put(strPair, 1);
						}
					}
					j++;
				}
		...
		public void cleanup(Context context) throws IOException, InterruptedException {
			for(Map.Entry<String, Integer> entry: localAggregate.entrySet()) {
