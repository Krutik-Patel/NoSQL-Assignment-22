...
public class StripsFuncLA {
	...
	public static class StripMapper extends Mapper<Object, Text, Text, Strip> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			Map<String, Strip> localAggregate = new HashMap<String, Strip>();
				...
				while(j <= d) {
					if (frequentIndices.contains(idx+j)) {
						...
						if (localAggregate.get(word1) != null) {
							Strip strip = localAggregate.get(word1);
			...
			for(Map.Entry<String, Strip> entry: localAggregate.entrySet()) {
	...
	public static class StripReducer extends Reducer<Text, Strip, Text, Strip> {
